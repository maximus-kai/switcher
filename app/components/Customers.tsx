...
const Customers = () => {
    ...
    return (
        <main className=' items-center py-10 lg:mx-[10%] xl:mx-[20%] md:mx-[8%] mx-[6%] px-5 mt-[2%] mb-[40px]  '>
            <div className='flex justify-between items-center mb-6'>
                <h3 className="text-center md:text-2xl font-semibold text-gray-800">
                    All Customers
                </h3>
            </div>
            <div className='grid grid-cols-1 sm:grid-cols-1 gap-4 md:grid-cols-1 lg:grid-cols-1'>
                {error && <div>Error: {error}</div>}
                {installations && installations.map((installation: Installation, index: number) => (
                    <div className='border rounded-lg shadow-md px-[80px] py-[40px] bg-gray-50' key={index}>
                        ...
                        <h2 className="">Radio Status: {installation.radioStatus}</h2>
